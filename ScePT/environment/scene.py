...
class Scene(object):
    def __init__(self, timesteps, map=None, dt=1, name="", frequency_multiplier=1, aug_func=None, 
                 non_aug_scene=None, x_min=None, y_min=None, x_max=None, y_max=None, map_name=None):
        self.map = map
    ...
    def sample_timesteps(self, batch_size, min_future_timesteps=0,min_history_timesteps=0) -> np.ndarray:
        """
        ...
        return np.random.choice(np.arange(min_history_timesteps, self.timesteps-min_future_timesteps), size=batch_size, replace=False)
